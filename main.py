    ...
    def moveCoin(self, event):
        ...
        if self.is_at_home():
            ...
            for i in range(roll[0] - 1):
                self.curr_index += 1
                self.canvas.coords(self.img, self.path_list[self.curr_index + 4][0] + 4,
                                   self.path_list[self.curr_index + 4][1] + 4)
            ...
            if check[0]:
                tkinter.messagebox.showinfo(
                    'INFO', 'You killed a coin so you get another chance.\nPlease roll the dice agan.')
                congrats = self.congratulations()
...
class Dice:
    ...
    def set(cls, flag):
        ...
        if colors[cls.chance][0].is_player_won():
            ...
            next_label = tk.Label(ludo.get_frame(), text='{}\'s turn'.format(turn[flag]), font=(None, 20), width=30,
                                  height=3, borderwidth=3, relief=tk.SUNKEN)
            next_label.place(x=60, y=100)

            roll_label = tk.Label(ludo.get_frame(), text='Please Roll.', font=(None, 20), width=30, height=3,
                                  borderwidth=3, relief=tk.RAISED)
            roll_label.place(x=60, y=200)

            img = ImageTk.PhotoImage(Image.open('./assets/trans.png'))
            image_label = tk.Label(ludo.get_frame(), width=95, height=95, image=img, bg=Color.CYAN)
            image_label.image = img
            image_label.place(x=230, y=320)
    ...
    def check_move_possibility(cls):
        check_1 = 0
        check_2 = 0
        for token in colors[cls.chance]:
            if token.is_at_home():
                check_1 += 1
            else:
                max_moves = len(token.path_list) - token.curr_index - 1
...
def startgame():
    ...
    start_label.place(x=60, y=100)
    top.destroy()


def create_enterpage():
    enter_label = tk.Label(top, text='Enter your name', font=(None, 20), width=20, height=2, borderwidth=3,
                           relief=tk.RAISED)
    enter_label.place(x=115, y=20)
    ...
    for i in range(2):
        ...
        temp.place(x=87, y=271 + i * 180)
        players.append(temp)

    for i in range(2):
        temp = tk.Entry(top, width=15)
        temp.place(x=387, y=450 - i * 180)
        players.append(temp)
...
def on_closing():
    if tkinter.messagebox.askokcancel("Quit message", "Do you want to quit the game? If you want to quit,"
                                                      " press OK."):
        top.destroy()
        root.destroy()
...
def on_closingroot():
    if tkinter.messagebox.askokcancel("Quit message", "Do you want to quit the game?"):
        root.destroy()
...
colors.append(
    align(11.1 * Board.SQUARE_SIZE, 2.1 * Board.SQUARE_SIZE, color='yellow', path_list=path.yellow_path, flag=3))
...
welcome_msg = 'Welcome to our game of ludo. Good luck and have fun.'

tkinter.messagebox.showinfo('Welcome message', welcome_msg)
